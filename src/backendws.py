...
def remove_conn_from_rooms(websocket : server.WebSocketServerProtocol):
  for room in rooms:
    rooms[room].servers.remove(websocket)
...
def process_message(websocket : server.WebSocketServerProtocol, rawmsg : wstypes.Data) -> bool:
  msg : Message = Message(rawmsg)
  
  if not msg.code:
    websocket.send(f"ERROR: Improperly formatted message.\n\n{rawmsg}")
  ...
  if msg.msg_type == "server_subscribe":
    ...
    if not rooms[msg.code].room_host:
      ...
      send_status_response(200, f"Joined room \"{msg.code}\" as host.")
      return True
    else:
      send_status_response(400, "Room already has a host.")
      return False
  elif msg.msg_type == "client_subscribe":
    if msg.code not in rooms:
      send_status_response(401, "Invalid room code.")
      return False
    if websocket not in rooms[msg.code].clients:
      rooms[msg.code].clients.append(websocket)
      send_status_response(200, f"Joined room \"{msg.code}\" as client.")
      return True
    else:
      send_status_response(409, f"Already in room \"{msg.code}\" as client.")
      return False
  elif msg.msg_type == "await_request":
    if msg.code not in rooms:
      send_status_response(websocket, 401, "Invalid room code.")
      return False
    if websocket not in rooms[msg.code].clients:
      send_status_response(401, f"Invalid room code.")
      return False
    else:
      ...
      rooms[msg.code].room_host.send(json.dumps(content))
      send_status_response(websocket, 200, f"{request_id}")
    ...
    if rooms[msg.code].room_host != websocket:
      ...
      for client in rooms[msg.code].clients:
        client.send(json.dumps(msg.data))
      send_status_response(200, "Broadcasted.")
      return True
    ...
    if websocket not in rooms[msg.code].clients:
      ...
      rooms[msg.code].room_host.send(json.dumps(content))
      send_status_response(websocket, 200, "Emitted.")
      return True
...
async def handler(websocket : server.WebSocketServerProtocol):
  while True:
    try:
      ...
      success = process_message(websocket, message)
    except:
      remove_conn_from_rooms(websocket)
      break
